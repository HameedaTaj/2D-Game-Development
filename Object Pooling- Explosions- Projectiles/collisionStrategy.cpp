...
const SDL_Color color = {240,232,112,255};

void RectangularCollisionStrategy::draw() const {
  IoMod::getInstance().writeText("Strategy: Rectangular", 460, 40,color);
}
...
void MidPointCollisionStrategy::draw() const {
  IoMod::getInstance().writeText("Strategy: Distance", 500, 30,color);
}
...
void PerPixelCollisionStrategy::draw() const {
  IoMod::getInstance().writeText("Strategy: Per-Pixel ", 500, 30,color);
}
